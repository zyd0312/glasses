...
import { SIGN_ALPHABET_REFERENCE_IMAGE_URL } from './sign-alphabet-reference.js';
...
const SYSTEM_PROMPT = [
  ...
  '识别英文字母时，必须以用户提供的参考字母表图片作为标准姿态。',
  '你必须只输出 JSON，不要输出 Markdown，不要解释 JSON 之外的内容。',
].join('');

const USER_PROMPT = [
  '下面会提供两张图片：第一张是手语英文字母参考图，第二张是待识别的相机照片。',
  '请以第一张参考图里的 A-Z 手势为规范，识别第二张照片中的单个静态手势。',
  '候选手势只有：',
  CANDIDATE_TEXT,
  ...
  const answer = await session.prompt([
    {
      ...
      content: [
        ...
        {
          type: 'image_url',
          image_url: { url: SIGN_ALPHABET_REFERENCE_IMAGE_URL },
        },
        {
          type: 'text',
          text: '上面是参考规范图。下面这张才是待识别的相机照片，请只输出第二张图片的识别结果。',
        },
        {
          type: 'image_url',
